  ...
  categorysData.forEach((category) => {
    cards += `
                  <div class="btn btn-lg hstack justify-content-between text-start text-light bg-component shadow">
                      <h4 class=" text-start fw-bold"> ${category.name} </h4>
                      <div class="hstack gap-2 justify-content-around">
                          <button onClick="handleUpdateClick(${categorysData.indexOf(
                            category
                          )},'${category.name}')" data-bs-toggle="modal"
                          data-bs-target="#updateFormModal" class="d-inline btn btn-sm btn-light"> ✏️ </button>
                          <button onClick="handleDeleteClick(${categorysData.indexOf(
                            category
                          )})" class="d-inline btn btn-sm btn-dark"> 🗑️ </button>
                      </div>
                  </div>
         `;
  });
